...
const ActionButtons = ({ player, gameState, currentPlayerId, players }) => {
  ...
  return (
    <div className="flex flex-col items-center space-y-4 max-w-md mx-auto">
      ...
      {/* 基础操作按钮 */}
      <div className="flex items-center space-x-3 bg-gray-800/95 backdrop-blur-sm px-6 py-3 rounded-xl border border-gray-600 shadow-lg">
        {/* 弃牌 */}
        <button
          onClick={() => handleAction('fold')}
          className="flex items-center justify-center w-12 h-12 bg-red-600 hover:bg-red-500 text-white rounded-xl transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105"
          title="弃牌"
        >
          <X size={20} />
        </button>
        ...
        {canCheck ? (
          <button
            ...
            className="flex items-center justify-center w-12 h-12 bg-green-600 hover:bg-green-500 text-white rounded-xl transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105"
            title="过牌"
          >
            <Check size={20} />
          </button>
          ...
          <button
            ...
            className="flex items-center justify-center px-6 h-12 bg-blue-600 hover:bg-blue-500 text-white rounded-xl transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105 text-sm font-bold"
            title={`跟注 ${callAmount}`}
          >
            跟注 {callAmount}
          </button>
        ...
        {canRaise && (
          <button
            ...
            className={`flex items-center justify-center w-12 h-12 ${showRaiseInput ? 'bg-yellow-500' : 'bg-yellow-600 hover:bg-yellow-500'} text-white rounded-xl transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105`}
            title="加注"
          >
            <TrendingUp size={20} />
          </button>
        ...
        {canRaise && (
          <button
            ...
            className="flex items-center justify-center px-4 h-12 bg-purple-600 hover:bg-purple-500 text-white rounded-xl transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-105 text-sm font-bold"
        ...
        <div className="w-full bg-gray-800/95 backdrop-blur-sm p-4 rounded-xl border border-gray-600 shadow-lg">
          ...
          <div className="mb-4">
            <input
              ...
              style={{
                background: sliderValue === player.chips 
                  ? `linear-gradient(to right, #a855f7 0%, #a855f7 100%)`
                  : `linear-gradient(to right, #f59e0b 0%, #f59e0b ${((sliderValue - gameState.minRaise) / (player.chips - gameState.minRaise)) * 100}%, #374151 ${((sliderValue - gameState.minRaise) / (player.chips - gameState.minRaise)) * 100}%, #374151 100%)`
              }}
            />
            <div className="flex justify-between text-xs text-gray-400 mt-1">
              <span>最小: {gameState.minRaise}</span>
              <span className={`font-medium ${sliderValue === player.chips ? 'text-purple-400 animate-pulse' : 'text-purple-400'}`}>
                All-in: {player.chips}
              </span>
            </div>
            ...
            {sliderValue === player.chips && (
              <div className="text-center text-purple-400 text-sm font-bold mt-1 animate-pulse">
                🚀 全押！
              </div>
            )}
          </div>
          ...
          <div className="text-center mb-3">
            <div className="text-lg font-bold text-yellow-400">
              {sliderValue === player.chips ? 'All-in' : `加注 ${sliderValue}`}
            </div>
            <div className="text-xs text-gray-400">
              {sliderValue === player.chips ? '全部筹码' : `总下注: ${sliderValue}`}
            </div>
          </div>
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className={`flex-1 h-10 ${sliderValue === player.chips ? 'bg-purple-600 hover:bg-purple-500' : 'bg-green-600 hover:bg-green-500'} text-white rounded-lg text-sm font-bold transition-all duration-200 shadow-md hover:shadow-lg`}
            >
