...
class ImprovedPotDistribution {
  ...
  static distributePot(gameLogic, result) {
    ...
    for (let i = pots.length - 1; i >= 0; i--) {
      ...
      // 找出符合该底池资格的获胜者
      const eligibleHands = playerHands.filter((hand) => pot.eligiblePlayers.includes(hand.player.id));
  ...
  static sendHandResult(gameLogic, result, playerWinnings, pots) {
    ...
    gameLogic.io.to(gameLogic.room.id).emit('handResult', {
      ...
      pots: pots.map((pot, index) => ({
        id: index,
        amount: pot.amount,
        winners: winners.filter((w) => pot.eligiblePlayers.includes(w.playerId)).map((w) => w.nickname),
      })),
